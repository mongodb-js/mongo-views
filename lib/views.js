(function (factory) {
    ...
    DBView.instantiate = function (target, name, query, projection, join) {
        ...
        if (join !== undefined) {
            ...
            for (var j in join) {
                ...
                makeTempJoinCollection.apply(this, newArgs);
            }
            // Override find in this case to query the temporary collection
            // TODO: what about drop? --CWS
            view.find = function() {
                var finalQuery = this._transformArgs(arguments);

                // return the find prototype with the merged query
                return db.getCollection(tmpCollName).find(finalQuery);
            };
    ...
    DBView.prototype._transformArgs = function() {
    ...
    DBView.prototype.find = function(){
        // TODO: Better merge this and _transformArgs, so joins and views can be in sync --CWS
        // transforming arguments
        var args = [].slice.call(arguments);
