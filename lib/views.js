(function (factory) {
    ...
    DBView.instantiate = function (target, name, query, projection, join) {
        ...
        if (join !== undefined) {
            ...
            view.find = function() {
                var mArgs = view._mergeFindArgs(arguments);

                var finalArgs = [mArgs.query, mArgs.projection].concat(mArgs.leftoverArgs);

                // return the find prototype with the merged query
                var target = db().getCollection(tmpCollName);
                return target.find.apply(target, finalArgs);
            };
            view.drop = function() {
                // delete local reference
                delete db()['_' + this._name];

                db().getCollection(config.VIEWS_COLLECTION_NAME).remove({ name: this._name });
                return db().getCollection(tmpCollName).drop();
            };
    ...
    function isCollectionOrView(name) {
        return ('_' + name in db()) || (db()[name].exists());
    }

    // Sanity checks on arguments to join.
    function validateJoinParams(joinArray) {
        if (!(joinArray instanceof Array)) {
            throw Error('Join parameter should be an array');
        }
        for (var j in joinArray) {
            var join = joinArray[j];
            if (!join.hasOwnProperty('foreignKey')) {
                throw Error('No foreignKey attribute specified for the join');
            }

            if (!join.hasOwnProperty('targets')) {
                throw Error('No targets attribute specified for the join');
            } else if (!(join.targets instanceof Array) || join.targets.length < 1) {
                throw Error('targets argument to join must be an array with at least one element.');
            } else {
                for (var i in join.targets) {
                    var target = join.targets[i];
                    ...
                    } else if (!isCollectionOrView(Object.keys(target)[0])) {
                        throw Error(Object.keys(target)[0] + ' is not an existing view or colleciton');
                    }
                }
            }
        }
