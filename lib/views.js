(function (factory) {
    ...
    DBView.instantiate = function (target, name, query, projection, join) {
        ...
        if (join !== undefined) {
            ...
            view.find = function() {
                var finalArgs;
                if (target instanceof DBView) {
                    var mArgs = target._mergeFindArgs(arguments);

                    finalArgs = [mArgs.query, mArgs.projection].concat(mArgs.leftoverArgs);
                } else {
                    finalArgs = [].slice.call(arguments);
                }
