...
DBView.prototype.createView = function(name, query, projection, join) {
    ...
    // persist
    var copiedJoin = Object.extend({}, join);
    if (copiedJoin.target) {
        // Save the name of the target, not the actual collection/view object
        copiedJoin.target = join.target.getName();
    }
    return db().getCollection(config.VIEWS_COLLECTION_NAME).insert(
        {
            ...
            join: JSON.stringify(copiedJoin)
        }
    );
};
...
function createTempCollection(from, to, fromKey, toKey) {
    ...
    tmpCollection.insert(fromResults.filter(function (fromDoc){
        return !!toObj[fromDoc[fromKey]];
    }).map(function (fromDoc) {
        // extract ids
...
function mergeProjections(first, second) {
    // TODO: Properly handle inclusion/exclusion of _id field
    var projection = {}, field;
    if (hasPropertiesWithValue(first, 1)) {
        projection['$'] = 1; // include impossible key to avoid empty projections
    ...
    // Remove the special key if it is not necessary
    if (Object.keys(projection).length > 1 && ('$' in projection)) {
        delete projection['$'];
    }
