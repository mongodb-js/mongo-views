(function (factory) {
    ...
    var getViewByName = internal.DB.prototype.getView = function(name) {
        ...
        if (doc) {
            return new DBView(getTargetByName(doc.target), doc.name, JSON.parse(doc.query), JSON.parse(doc.projection), deserializeJoin(doc.join));
        }
    };
    ...
    DBView.prototype.findOne = internal.DBCollection.prototype.findOne;

    // count is implemented in terms of find, so just reuse the DBCollection implementation
    DBView.prototype.count = internal.DBCollection.prototype.count;
